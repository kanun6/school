...
export async function GET() {
  ...
  // อ่าน cookies ใหม่และสร้าง supabase client สำหรับการ query
  const cookieStore = await cookies();
  ...
  let query = supabase
    .from('issues')
    .select(`*, reported_by:profiles(first_name, last_name, role)`)
...
export async function POST(request: Request) {
  ...
  // อ่าน cookies ใหม่และสร้าง supabase client สำหรับการ insert
  const cookieStore = await cookies();
  const supabase = createServerClient(
    process.env.NEXT_PUBLIC_SUPABASE_URL!,
    process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY!,
  ...
  try {
    ...
    const { error } = await supabase
      .from('issues')
      .insert({ title, description, category, reported_by: user.id });
