...
// กำหนดชนิดของสถานะให้เป็นยูเนียน (หรือจะใช้ keyof colorClasses ก็ได้)
type IssueStatus = 'ใหม่' | 'กำลังดำเนินการ' | 'แก้ไขแล้ว';

const StatusBadge = ({ status }: { status: IssueStatus }) => {
  // กำหนด type ให้กับ colorClasses เพื่อให้ TS รู้ว่าคีย์เป็น IssueStatus
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (res.ok) {
      ...
      await showAlert({
        ...
        message: data.error || 'ไม่สามารถส่งเรื่องได้',
        type: 'alert',
      });
    }
    setLoading(false);
    ...
    <div className="space-y-8">
      <div>
        ...
        <p className="mt-2 text-gray-500 dark:text-gray-400">
          พบเจอปัญหา? แจ้งให้เราทราบได้ที่นี่
        </p>
      </div>
